...
TMPDIR = _ipc_dir()
SENSE_PATHS = [
    ROOT / "fly_fnf_sense.txt",
    TMPDIR / "fly_fnf_sense.txt",
    Path("mods/fruit-fly/python/fly_fnf_sense.txt"),
    Path("./mods/fruit-fly/python/fly_fnf_sense.txt"),
]
PRESS_PATHS = [
    ROOT / "fly_fnf_press.txt",
    TMPDIR / "fly_fnf_press.txt",
    Path("mods/fruit-fly/python/fly_fnf_press.txt"),
    Path("./mods/fruit-fly/python/fly_fnf_press.txt"),
]
SENSE = SENSE_PATHS[0]
PRESS = PRESS_PATHS[0]
...
class MiniBrain:
    def __init__(self, path: Path):
        ...
        self.w_rec = rng.normal(0.0, 0.045, size=(extra, extra)).astype(np.float32)
    ...
    def step(self, etas, combo=0):
        ...
        for s, t, w in self.fast:
            if 0 <= s < self.n and 0 <= t < self.n:
                acc[t] += max(self.hidden[s], 0.0) * w
...
def commit(motor, bump, due, sustain, held):
    ...
    for i in range(4):
        lit = float(motor[i]) >= MOTOR_GO and float(bump[i]) >= BUMP_GO
        near = float(bump[i]) >= BUMP_GO and float(motor[i]) >= MOTOR_GO * 0.5
        if sustain[i]:
            bits[i] = True
        elif due[i] and (lit or near):
            bits[i] = True
    return bits
...
def run():
    ...
    print("It also closes itself once the game is gone.")
    while True:
        ...
        elif saw_game and cycle_start - last_game > GAME_GONE:
        ...
        if sense.get("song"):
            ...
            brain.hidden *= 0.92
        held[:] = bits
        if sense.get("keys") and sense.get("song"):
            focus_game_window()
            keys.set_held(bits)
        else:
            keys.release_all()
        ...
        if key in (27, ord("q"), ord("Q")):
            break
        if frame % DRAW_EVERY == 0 and window_gone(WINDOW_NAME):
            break
